...
all: protodecode/protodemo/demo.pb.go $(NODE_DIR)
	...
	# ensure Go code is formatted and modernized
	goimports -l -w .
	go fix ./...
	go mod tidy
	...
	# format protocol buffers and C code
	git ls-files '**.proto' | xargs clang-format -Werror -i '-style={ColumnLimit: 100}'
	git ls-files '**.c' | xargs clang-format -Werror -i '-style={ColumnLimit: 100}'
